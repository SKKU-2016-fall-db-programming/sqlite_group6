...
static sqlite3_pcache *pcache1Create(int szPage, int szExtra, int bPurgeable){
  ...
  if( pCache ){
    ...
    if( pGroup->lru.isAnchor==0 ){
      ...
      pGroup->midPoint = &pGroup->lru; //FIXME
    }
  ...
  if( pPage ){
    ...
    */
    ...
    if(pGroup->midPoint==&pGroup->lru){
      pGroup->lru.pLruPrev = pGroup->lru.pLruNext = pPage;
      pPage->pLruPrev = pPage->pLruNext = &pGroup->lru;
    }else{
      pPage->pLruNext=pGroup->midPoint;
      pPage->pLruPrev=pGroup->midPoint->pLruPrev;
